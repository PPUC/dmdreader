...
/**
 * Derived from https://github.com/pico-coder/sigrok-pico
 */
...
// because it's not clear how to define RLE on analog signals
...
// See if a given half's dma engines are idle and if so process the data, update
// the write pointer and ensure that when done the other dma is still busy
// indicating we didn't lose data . int
...
int check_half(sr_device_t *d, volatile uint32_t *tstsa0,
  ...
  if (((a0busy == 0) || (d->a_mask == 0)) &&
    ...
    // When one half finishes we send it's data while the other dma controller
    // writes to the other half. We rely on DMA chaining where one completing
    // dma engine triggers the next. When chaining happens the original target
    // address is not restored so we must rewrite the starting address. Also
    // when chaining is enabled we can get into an infinite loop where we ping
    // pong between each other. and it is possible that if the other DMA
    // controller finishes before we have send our half of the data that our DMA
    // controller will startup and overwrite our data.  That is the DMA overflow
    // condition. We don't actually overflow the DMA, but instead we fail to
    // establish proper chaining to keep the ping pong going. The only way to
    // avoid the overflow condition is to reduce the sampling rate so that the
    // transmit of samples can keep up, or do a fixed sample that fits into the
    // sample buffer. When we first enter the loop it is assumed that the other
    // controller is not chained to us so that if we don't process in time then
    // our DMA controller won't be started by the other.  That assumption is
    // maintained by always pointing our DMA engine to itself so tht in the next
    // function call it is the "other" controller. We then process our half's
    // samples and send to USB. When done we confirm that the other channel is
    // still active, and if so we know we haven't overflowed and thus can
    // establish the chain from the other channel to us.  If the other channel
    // is not still active, or if the PIO/ADC FIFOs indicates an RXstall
    // condition which indicates PIO lost samples, then we abort. Note that in
    // all cases we should never actually send any corrupted data we just send
    // less than what was requested. Note that we must use the "alias" versions
    // of the DMA CSRs to prevent writes from triggering them. Since we swap the
    // csr pointers we determine the other half from the address offsets.
...
// 0, but doing so did not improve streaming performance, so it's left in core0.
...
void core1_code() {
  ...
  while (true) {
    ...
    // Each core instruction takes a memory cycle, as does each core memory or
    // IO register read. The memory fabric supports up to 4 reads per cycle if
    // there are no bank conflicts The DMA engine needs a read of the PIO and
    // ADC and a write of memory and thus chances of conflicts are high.  Thus
    // once we are in sampling mode slow down C1 to allow C0 to loop faster and
    // process faster. Without wfes C1 typically completes 10x the number of
    // outer loops as C0, but with the wfes C0 completes more loops and C1
    // activity drops to 1% . Much of the C0 code has built in sev (Send Event)
    // but we also add an explicit one in the main while loop
    ...
    // can end a continuous trace.  A reset '*' should only be seen after we
    // have completed normally or hit an error condition.
    if (intin == '+') {
      dev.sending = false;
  ...
  */
  ...
  while (1) {
    ...
    if (dev.sending && (dev.started == false)) {
      // Only boost frequency during a sample so that average device power is
      // less. It's not clear that this is needed because rp2040 is pretty low
      // power, but it can't hurt...
#ifdef SYS_CLK_BOOST_EN
      // This is not enabled if any analog channels are enabled because due to
      // ADC frequency limits the processing rate is never the limiter to
      // performance. And maybe it might help avoid an overfrequency issue with
      // ADC...or not...
      if (dev.a_chan_cnt == 0) {
        Dprintf("Boost up\n\r");
      ...
      *adcfcs |= 0xC00;
      ...
      // Enable the initial chaing from the first half to 2nd, further chains
      // are enabled based on whether we can parse each half in time.
      ...
      if (dev.a_chan_cnt) {
        ...
        // set chan0 to immediate trigger (but without adc_run it shouldn't
        // start), chan1 is chained to it.
       ...
       */
        ...
        //                       number    config   buffer target piosm xfer
        //                       size  trigger
        ...
        // This is done later so that we start everything as close in time as
        // possible
        //              pio_sm_set_enabled(pio, piosm, true);
      }  // dev.d_mask
      // These must be at their initial value,(or zero for the 2ndhalf)
      // otherwise it indicates they have started to countdown Dprintf("Tcount
      // start d %u %u a %u %u\n\r",*tcountd0,*tcountd1,*tcounta0,*tcounta1);
      // These must be the initial value for both
      // Dprintf("Tcount dbg start d 0x%X 0x%X a 0x%X
      ...
      // %d\n\r",admachan0,admachan1,pdmachan0,pdmachan1); Dprintf("DMA ctr reg
      // addrs a %p %p d %p %p\n\r",(void *) tstsa0,(void *)tstsa1,(void
      // *)tstsd0,(void *)tstsd1); Dprintf("DMA ctrl reg a 0x%X 0x%X d 0x%X
      // 0x%X\n\r",*tstsa0,*tstsa1,*tstsd0,*tstsd1); Enable logic and analog
      // close together for best possible alignment warning - do not put printfs
      // or similar things here...
      tstart = time_us_32();
    ...
    // In high verbosity modes the host can miss the "!" so send these until it
    // sends a "+"
    if (dev.aborted == true) {
    ...
    if ((dev.sending == false) && (init_done == true)) {
      ...
      // 0x%X\n\r",*pioctrl,*piofstts,*piodbg,*pioflvl); The end of sequence
      // byte_cnt uses a "$<byte_cnt>+" format. Send the byte_cnt to ensure no
      // bytes were lost
      ...
      // USBCTRL 0x0 is setup packet, 0x8-0xfc - EP in/out buffer control 0x100
      // - EP0 buffer 0 (in and out) 0x140 - EPO buffer 1 0x180 - data buffers
      /*
      volatile uint32_t *usbctrl;
      usbctrl=(volatile uint32_t *)(USBCTRL_BASE);
